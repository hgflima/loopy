...
2. **Tipos e lógica pura (build-time): subpath imports `loopy/…`.** Hoje são **seis**: `loopy/tui/{store,view,transport}` + **`loopy/config`** (schema zod, `parseConfigSource`, `serializeConfig`, `initialConfigTemplate`) + **`loopy/backlog`** (`parseBacklog`) + **`loopy/scheduler`** (`resolveConcurrency`/`maxLayerWidth`/`topoLayers` — browser-safe, ADR-0009) — mais `loopy/types`. É o que permite editar e validar o `loopy.yml` **sem reimplementar nada**.
...
- Não duplicar no frontend o que o motor já exporta: **reducer, status de task, schema zod do yml, tipos de step, parser do backlog, resolveConcurrency**. Duplicar é como app e motor divergem em silêncio.
...
- Decisão: `docs/adrs/0007-transport-ndjson-duplex-native-ui.md`, `docs/adrs/0008-capabilities-de-agente-por-descoberta.md` (sondagem/selects sondados), `docs/adrs/0009-concorrencia-derivada-do-dag.md` (scheduler subpath). Produto/design do app: `PRODUCT.md`, `DESIGN.md` (este último é **target**, não o estado atual do código).

## Patterns & Pitfalls
- **Sondagem e selects sondados (ADR-0008):** o `ConfigPane` tem presets de `command` (Claude / Codex / OpenCode / Em branco — D27) e um botão "sondar/refresh" (D32) que chama `probe-agent` e popula os selects de `mode`/`model`/`effort` com os **valores reais** daquele Agente. O `StepEditor` usa `useAgentCapabilities` para renderizar `SelectField` quando as capabilities estão disponíveis; degrada para `TextField` (texto livre) quando a sondagem falha (D31) — e um valor já presente que não está na lista do probe **não é corrompido** ao salvar (`ensureOption`).
- **A sondagem vai pelo ARGV, não pelo nome do agente** (D-0011): `useAgentCapabilities` manda `probe-agent --command <argv> --env K=V`, montado do **draft**. Sondar por nome resolvia contra o **yml salvo** — então um agente com o `preset` recém-trocado era sondado como o adapter *antigo*, e a tela mostrava as capabilities erradas **sem falhar**. Trocar o preset também **descarta `model`/`effort`** (são Dialeto por-Agente) e semeia os **defaults sondados** do agente novo (`defaultModel`/`defaultEffort`).
- **A sondagem é por `(agente, model)`, não por agente.** `useAgentCapabilities` recebe o model em vigor (`agent.model` no `ConfigPane`; `step.model ?? agent.model` no `StepEditor`) e o repassa ao `probe-agent --model`; trocar o model **re-sonda**, e a lookup no cache usa a key `argv::model`. Isso existe porque no OpenCode o `effort` são as *variants do model corrente* — sondar pelado responde "sem effort" para um agente que tem effort. Por isso o motivo do `effort` desabilitado tem **três** formas (`config/capability-hints.ts`): sem model escolhido / o model não tem níveis / o agente não anuncia effort. Nunca diga "este agente não anuncia effort" sem olhar o model.
- **`concurrency: auto` no `ConfigPane` (ADR-0009):** toggle "auto (derivar do DAG)" que alterna `concurrency` entre `"auto"` e `1`. Com `auto`, mostra `max_concurrency`; com número, mostra `concurrency`. O `DepsFlow` resolve o `auto` usando `resolveConcurrency` do subpath `loopy/scheduler` (a **mesma** função do motor).
