...
## Purpose & Scope
A casca macOS do app: ciclo de vida do processo (accessory app, sem Dock), tray icon, as 3 janelas, o painel do popover (NSPanel swizzled) e o **supervisor do sidecar** — o subprocesso `loopy` que é o Motor. É aqui que moram as armadilhas mais caras do app (resolução do binário, PATH, geometria do popover). **Não conhece o domínio do loop**: nunca parseia NDJSON de evento, nunca sabe o que é Task/Step — só line-frama stdout e repassa ao webview.
...
- **14 comandos** `#[tauri::command]` (registrados no final de `main.rs`). Os que carregam contrato: `start_sidecar(dir, flags)`, `send_command(payload)` (escreve **uma linha NDJSON no stdin** do sidecar), `stop_sidecar()`, `resize_popover(height)`, `update_tray_title(title)`, `load/save_launch_config`, `log_error(...)`, `quit_app`.
...
- Testes: unitários inline (`#[cfg(test)]`) em `main.rs`, `sidecar.rs`, `panel.rs`, `config.rs`. Rodam com `cargo test` — **nenhum script npm os invoca**, e o ESLint/`tsc` ignoram este diretório inteiro.
