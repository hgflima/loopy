...
- **16 comandos** `#[tauri::command]` (registrados no final de `main.rs`). Os que carregam contrato: `start_sidecar(dir, flags)`, `send_command(payload)` (escreve **uma linha NDJSON no stdin** do sidecar), `stop_sidecar()`, `resize_popover(height)`, `update_tray_title(title)`, `load/save_launch_config`, **`read_project_files(dir)`** e **`write_loopy_yml(dir, contents)`** (C-0014), `log_error(...)`, `quit_app`.
- **`project_fs.rs`** — a ponte de FS com o projeto-alvo. Contrato serde snake_case: `read_project_files` devolve `{ loopy_yml, todo_md }` como `Option<String>` — **arquivo ausente vira `None`, erro de I/O vira `Err`**. O frontend depende dessa distinção para escolher entre empty-state e mensagem de erro. `write_loopy_yml` **faz backup antes de sobrescrever** (`<dir>/.loopy/backups/loopy.<epoch>.yml`, retenção 10). São comandos dedicados **em vez de `tauri-plugin-fs`** justamente para não abrir permissão de FS ao webview — mesmo raciocínio do `hide_popover`.
