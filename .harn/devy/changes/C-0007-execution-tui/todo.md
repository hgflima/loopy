...
- [ ] T-001: Store — evento `acp_traffic` + `acpLog` ring bounded + fallback `--verbose`
    ...
    Deps: nenhuma. Files: src/tui/store.ts, src/tui/line-reporter.ts, src/tui/start.ts, testes. Scope: M.
...
- [ ] T-002: View — remap `COLORS.task` (amarelo=aguardando) + `pulseFrame`
    ...
    Verificação: `npm test -- tui` && `npm run typecheck`.
    Deps: nenhuma. Files: src/tui/view.ts, src/tui/view.test.ts. Scope: S.
...
- [ ] T-003: View — `layoutGraph` (`@dagrejs/dagre`) + `renderGraph` → `GraphGeometry` PURA
    ...
    Verificação: `npm test -- tui` && `npm run typecheck`.
    Deps: T-002. Files: src/tui/view.ts, src/tui/view.test.ts, package.json. Scope: M. RISCO ALTO.
...
- [ ] T-004: Orquestrador — `OrchestratorDeps.emit` + `StepContext.emit` (espelha transições)
    ...
    Deps: nenhuma. Files: src/types.ts, src/loop/orchestrator.ts, testes. Scope: L. RISCO ALTO.
...
- [ ] T-005: Checks ao vivo — `onCheckStart/onCheckEnd` + `attempt_started`/`check_*`
    ...
    Deps: T-004. Files: src/types.ts, src/checks/runner.ts, src/steps/agent.ts, src/steps/checks.ts, testes. Scope: M.
...
- [ ] T-006: Shell stream — `onChunk` aditivo → `stream_chunk` ao vivo
    ...
    Deps: T-004. Files: src/steps/shell.ts, testes. Scope: M.
...
- [ ] T-007: ACP — `onTraffic` (send+recv) + `onUpdate` seams (observação pura, AD-1)
    ...
    Deps: nenhuma. Files: src/acp/agent.ts, src/acp/client.ts, src/acp/session.ts, src/logging/logger.ts, testes. Scope: M.
...
- [ ] T-008: Entrypoint — mount Ink + `emit=dispatch` + `sessionId→taskId` + logs arquivo-only
    ...
    Deps: T-001, T-004, T-007. Files: src/index.ts, testes. Scope: L. RISCO ALTO.
...
- [ ] T-009: Panes — `GraphPane` + `AcpLogPane` + `TaskListPane` (+`ink-testing-library`)
    ...
    Verificação: `npm test -- tui` && `npm run typecheck`.
    Deps: T-001, T-002, T-003. Files: src/tui/components/GraphPane.tsx, src/tui/components/AcpLogPane.tsx, src/tui/components/TaskListPane.tsx, testes, package.json. Scope: M.
...
- [ ] T-010: `App.tsx` → Dashboard fixo (header + Grafo + split Tasks | Stream+Logs) + pulso
    ...
    Verificação: `npm test -- tui` && `npm run typecheck`.
    Deps: T-009. Files: src/tui/App.tsx, src/tui/App.test.tsx, testes. Scope: M.
...
- [ ] T-011: ADR-0005 + CONTEXT.md (glossário) + `src/tui/CLAUDE.md` + full green
    ...
    Deps: T-001, T-002, T-003, T-004, T-005, T-006, T-007, T-008, T-009, T-010. Files: docs/adrs/0005-*.md, docs/adrs/README.md, CONTEXT.md, src/tui/CLAUDE.md. Scope: M.
