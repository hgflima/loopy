...
- > TODO(intent): bug — os eventos `check_started`/`check_finished` **nunca são emitidos num run real**. O `ChecksRunnerPort` de produção é construído inline em `../index.ts` e só repassa `cwd`, descartando os callbacks; o `createChecksRunner` que os encaminha existe em `../checks/runner.ts` e não é usado.
- > TODO(intent): bug — `cancelSignal` é seam morto: `shell.ts` implementa hard-stop por ele, mas o campo não existe em `NonAgentRegistryOptions` e nunca é passado no wiring.
- > TODO(intent): bug — `approval.ts` interpola o `on_fail` cru no template (`on_fail: ${onFail}`), então `on_fail: { goto }` renderiza `[object Object]`. `agent.ts` usa `formatOnFail` e acerta.
