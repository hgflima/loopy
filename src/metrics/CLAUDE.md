...
- **Débito aberto** (`.harn/devy/debts/`): **D-0008** — o custo de **Run**/**Change** ser `last-non-null` enquanto o de **Task** virou soma **subconta**: o "Total Run · custo" mostra o custo da última Task iterada, não a soma. `types.ts` e o docstring de `folds.ts` ainda afirmam a regra antiga. O custo **por Task** é confiável; o total agregado não.
