...
- **Débito aberto** (`.harn/devy/debts/`): **D-0004** — `start.ts` emite o control frame `approval_requested` com **`taskId: ""` e `stepId: ""` hardcoded**, embora o frame declare ambos. A GUI casa a decisão pelo `requestId` (então funciona), mas fica sem âncora para associar o gate à Task.
