...
## Patterns & Pitfalls
- **Dashboard vivo (ADR-0005):** num TTY real (sem `--no-tui`, com `mount` injetado) o `App.tsx` monta o Dashboard fixo — Grafo (layout dagre, **~70% da altura**) no topo, split Tasks | Streams abaixo — e o progresso flui pela store. Em modo TUI o logger é **arquivo-only** (o tee no stdout corromperia o frame) e o `notify` (escalação/dirty-parent) é bufferizado e drenado ao stderr **após** `ui.stop()`.
