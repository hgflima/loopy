...
- **ACP significativo (`AcpLogPane` + `store` + `index.ts`):** `session/update` cru não tem sinal; `acpTrafficSummary` (em `acp/client.ts`) expõe o **sub-tipo** (`agent_message_chunk`/`tool_call`/…) como summary, o reducer **colapsa eventos idênticos consecutivos** num `count` (`×N`), e o pane pinta RPCs estruturais (prompt/set_mode/permission/fs/terminal) em amarelo vs. os updates streamy esmaecidos. Prefixo de `taskId` só sob concorrência.
