...
- **Loop externo** (itera Tasks do Backlog; contador = **Iteração**, teto `max_iterations`) × **Loop interno** (o Verify de um Step de Agente; contador = **Tentativa**, teto `max_attempts`). **Visita** = cada entrada num Step pelo PC (teto `max_step_visits`). Nunca troque Iteração ↔ Tentativa ↔ Visita.
