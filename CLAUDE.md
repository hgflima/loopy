...
- **Segundo contrato público**: os subpath exports `@hgflima/loopy/tui/{store,view,transport}` (buildados com `dts`). São a API que a GUI consome — mudar a shape de `StoreEvent` ou de um frame quebra `apps/menubar/`.
